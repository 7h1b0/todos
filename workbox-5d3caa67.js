define('./workbox-5d3caa67.js', ['exports'], function (e) {
  'use strict';
  try {
  ...
  const a = (e, t) => e.filter((e) => t in e),
    l = async ({
      ...
      const c = await self.caches.open(e),
        r = await h({ plugins: i, request: t, mode: 'read' });
    ...
    u = async ({ request: e, response: t, event: n, plugins: s = [] }) => {
      let i = t,
        c = !1;
      for (let t of s)
    ...
    f = async ({
      ...
      const w = await h({ plugins: c, request: t, mode: 'write' });
      if (!n) throw new r('cache-put-with-no-response', { url: i(w.url) });
      let d = await u({ event: s, plugins: c, response: n, request: w });
      if (!d) return;
      const p = await self.caches.open(e),
        y = a(c, 'cacheDidUpdate');
      let g =
        y.length > 0
          ? await l({ cacheName: e, matchOptions: f, request: w })
          : null;
      try {
        await p.put(w, d);
      } catch (e) {
    ...
    w = async ({ request: e, fetchOptions: t, event: n, plugins: s = [] }) => {
      if (
      ...
      try {
        ...
        throw new r('plugin-error-request-will-fetch', { thrownError: e });
      }
      let o = e.clone();
      try {
        let i;
  ...
  class g {
    ...
    async install({ event: e, plugins: t } = {}) {
      ...
      for (const [e, t] of this.s)
        r.has(t) ? s.push(e) : n.push({ cacheKey: t, url: e });
      const o = n.map(({ cacheKey: n, url: s }) => {
    ...
    async l({
      ...
      let a,
        l = await w({ event: s, plugins: i, request: o });
      for (const e of i || []) 'cacheWillUpdate' in e && (a = e);
      if (
        !(a
          ? await a.cacheWillUpdate({ event: s, request: o, response: l })
          : l.status < 400)
      )
        throw new r('bad-precaching-response', { url: t, status: l.status });
      l.redirected && (l = await p(l)),
        await f({
          event: s,
          plugins: i,
          response: l,
...
//# sourceMappingURL=workbox-5d3caa67.js.map
